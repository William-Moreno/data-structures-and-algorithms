...
const templatingWithMustache = () => {
  ...
  characters.forEach(person => {
    ...
    const outputHtml = Mustache.render($template, person);
    templateArray.push(outputHtml);
  });
...
const getHouses = (arr) => {
  ...
  arr.forEach(person => {
    ...
    houses.push(valueArray[valueArray.length-1]);
  });
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  let answer = false;
  arr.forEach(person => {
    let parentArray = Object.values(person);
    if(parentArray.includes(character)){
      let childrenArray = Object.values(parentArray[2]);
      if(childrenArray[0]){
        answer = true;
      }
    }
