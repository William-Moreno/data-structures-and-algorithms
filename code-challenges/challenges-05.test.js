...
const howMuchPencil = (str) => {
  ...
  for(let i = 0 ; i <= str.length ; i++){
    let reduction = str.slice(i, str.length + 1);
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(entry => {
    let stepOne = entry.slice((entry.indexOf(' ') + 1), entry.length + 1);
    let stepTwo = stepOne.slice((stepOne.indexOf(' ') + 1), stepOne.length + 1);
    result.push(stepTwo);
  });
  return result;
};
...
const extractVowels = (str) => {
  const vowelExtract = [];
  const voweless = str.replace(/[aeiou]/gi, '');
  vowelExtract.push(voweless);
  let onlyVowels = str.replace(/[bcdfghjklmnpqrstvwxyz\s]/gi, '');
  let alphaVowels = onlyVowels.split('');
  alphaVowels.sort((a, b) => {
    if( a > b ){
      return 1;
    } else if( a < b ){
      return -1;
  ...
  onlyVowels = alphaVowels.join('');
  vowelExtract.push(onlyVowels);
