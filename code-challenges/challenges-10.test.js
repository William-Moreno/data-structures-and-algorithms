...
const createServer = () => {
  ...
  app.get('/hello', (req,res) => res.send('Hello. How are you?'));

  app.get('/aboutme', (req,res) => res.send('I own way too many games...'));

  app.get('/favoritefoods', (req,res) => res.send(['pizza', 'tacos', 'lasagna', 'enchiladas', 'soft pretzels']));
...
const count = (target, input) => {
  ...
  for(let i = 0 ; i < input.length ; i++){
    let counter = input[i].reduce((accum, val) => {
      if(val === target){
        accum++;
      }
      return accum;
...
const totalSum = (input) => {
  let totalAmount = input.reduce((accum,val) => {

    let counter = val.reduce((inAcc, inVal) => {
      return inAcc + inVal;
    }, 0);
